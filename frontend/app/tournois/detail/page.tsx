...
export default function TournamentDetailPage() {
  return (
    <main className="min-h-screen bg-[#fbfbfd] text-[#080b15]">
      <SiteHeader />
      <section className="mx-auto grid w-full max-w-[1700px] grid-cols-[minmax(0,1fr)_320px] gap-4 px-6 py-4">
        <div className="min-w-0">
          <section className="grid grid-cols-[330px_minmax(0,1fr)_270px] overflow-hidden rounded-lg border border-[#edf0f4] bg-white shadow-[0_2px_12px_rgba(16,24,40,.05)]">
            <img src={cupImage} alt="" className="h-[330px] w-full object-cover" />
            <div className="p-5">
              ...
              <h1 className="text-2xl font-black">NEXY CUP #12 - GRANDE FINALE</h1>
              <div className="mt-4 flex flex-wrap gap-5 text-xs text-[#4b5563]">
              ...
              <p className="mt-6 max-w-3xl text-xs leading-5 text-[#4b5563]">La grande finale de la NEXY CUP #12. Les meilleures équipes s'affrontent pour le titre et une récompense exceptionnelle.</p>
              <div className="mt-8 rounded bg-[#f8fafc] p-4">
                <p className="text-[10px] font-black uppercase">Récompense totale</p>
                <div className="mt-2 flex items-center gap-8 text-sm"><b className="text-xl">💎 100,000</b><span>🎁 + Trophées exclusifs</span></div>
              </div>
            </div>
            <aside className="border-l border-[#edf0f4] p-5 text-xs">
          ...
          <nav className="mt-4 flex overflow-hidden rounded-lg border border-[#edf0f4] bg-white text-[11px] font-black shadow-[0_2px_10px_rgba(16,24,40,.04)]">
            {tabs.map(({ label, Icon }, i) => <button key={label} className={`relative flex h-11 flex-1 items-center justify-center gap-2 px-4 ${i === 0 ? "text-[#6d28d9]" : "text-[#374151]"}`}><Icon className="h-3.5 w-3.5" />{label}{i === 0 && <span className="absolute bottom-0 left-5 right-5 h-0.5 rounded-full bg-[#6d28d9]" />}</button>)}
          </nav>

          <section className="mt-4 grid grid-cols-[280px_minmax(0,1fr)] gap-4">
            <div className="rounded-lg border border-[#edf0f4] bg-white p-4 text-xs shadow-[0_2px_10px_rgba(16,24,40,.04)]">
              ...
              <div className="overflow-hidden rounded border border-[#edf0f4]">
                {details.map(([label, value]) => <div key={label} className="grid grid-cols-[1fr_1.25fr] border-b border-[#edf0f4] last:border-b-0"><span className="bg-[#fbfbfd] px-3 py-2.5 text-[#6b7280]">{label}</span><b className="px-3 py-2.5 text-right">{value}</b></div>)}
              </div>
            </div>
            <div className="rounded-lg border border-[#edf0f4] bg-white p-4 text-xs shadow-[0_2px_10px_rgba(16,24,40,.04)]">
              ...
              <div className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2">
                {schedule.map(([label, date, time, status], index) => <div key={label} className="relative flex items-center gap-3 rounded-lg border border-[#edf0f4] bg-white p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <b>{label}</b>
                    <p className="mt-1 flex items-center gap-3 text-[#6b7280]"><span className="flex items-center gap-1"><CalendarDays className="h-3.5 w-3.5" />{date}</span><span className="flex items-center gap-1"><Clock3 className="h-3.5 w-3.5" />{time}</span></p>
                  </div>
          ...
          <section className="mt-4 rounded-lg border border-[#edf0f4] bg-white p-4">
            ...
            <div className="grid grid-cols-5 gap-4">
              {rewards.map(([place, amount, label, icon]) => <article key={place} className="rounded bg-[#f8fafc] p-4"><div className="text-3xl">{icon}</div><p className="mt-2 text-[10px] font-black uppercase text-[#6b7280]">{place}</p><b>{amount}</b><p className="text-xs text-[#6b7280]">{label}</p></article>)}
            </div>
          </section>
        </div>

        <aside className="sticky top-4 max-h-[calc(100vh-32px)] space-y-4 overflow-y-auto">
