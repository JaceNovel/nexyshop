...
class FreeFireLookupService
{
    ...
    {
        $this->accountEndpoint = (string) config('services.freefire.lookup.endpoint');
        $this->likesEndpoint = config('services.freefire.lookup.likes_endpoint');
        $this->apiKey = config('services.freefire.lookup.api_key');
    }
    ...
    public function validateUid(string $uid, string $region): array
    {
        ...
        if (! $this->configured()) {
            throw new RuntimeException('Service Free Fire indisponible.');
        }

        $cacheKey = "freefire:lookup:validation:{$region}:{$uid}";
    ...
    public function accountInfo(string $uid, string $region): array
    {
        ...
        $cacheKey = "freefire:lookup:account:{$region}:{$uid}";
    ...
    {
        $region = $this->normalizeRegion($region);
        $cacheKey = "freefire:lookup:profile:{$region}:{$uid}";
    ...
    {
        $maxLikes = (int) config('services.freefire.likes_daily_limit', 100);
        $price = (int) config('services.freefire.likes_price_xof', 300);
    ...
    {
        return Http::timeout((int) config('services.freefire.lookup.timeout', 12))
            ->retry(2, 300)
    ...
    private function providerRegion(?string $region): ?string
    {
    ...
    {
        $query = ['uid' => $uid];
        $apiRegion = $this->providerRegion($region);
